...
export default {
  ...
  seasonFarms: (local, state) => {
    let farms;
    if (_.get(state, `OADAManager.connected`) === true) {
      let currentConnection = _.get(state, `OADAManager.currentConnection`)
  ...
  currentOADA: (local, state) => {
    if (state.OADAManager.connected) {
      const currentConnection = state.OADAManager.currentConnection;
